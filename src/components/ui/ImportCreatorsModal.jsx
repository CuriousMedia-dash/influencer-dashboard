...
export default function ImportCreatorsModal({ open, onClose }) {
  ...
  // Non-admins never see the Master Sheet tab at all — Google Sheet
  // syncing is entirely an admin capability. They land straight on the
  // only tab they can actually use.
  ...
  // ── Master sheet handlers (admin only) ──
  ...
  return (
    <Modal
      ...
      description={
        tab === TABS.UPLOAD
          ? "Export your Google Sheet (or any spreadsheet) as a CSV file and upload it here to add new creators or update existing ones. Rows are matched by their platform link — this can never delete anyone, only add or update."
          : "This is the one master sheet the whole team syncs from — everyone sees the same data. Rows are matched by their platform link."
      }
      maxWidth={520}
    >
      {isAdmin && (
      ...
      {/* ══════════════ MASTER SHEET TAB (admin only) ══════════════ */}
      {tab === TABS.LINK && isAdmin && (
        <>
