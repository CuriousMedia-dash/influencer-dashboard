...
export default function DeckEditorModal({ open, onClose, recipients }) {
  ...
  async function buildPptx() {
    ...
    slides.forEach((slide) => {
      ...
      // Full-bleed — the slide IS the image, exactly as designed.
  ...
  return (
    <Modal open={open} onClose={onClose} title="Forward Mail — Deck" maxWidth={760}>
      <div className="flex flex-col gap-3 p-1">
        <div className="text-[12px]" style={{ color: "var(--ink3)" }}>
          ...
          actual deck, exactly as designed. Swap a slide's image below if you need to, or send as-is.
        </div>
        ...
        <div className="overflow-hidden rounded-[10px] border" style={{ borderColor: "var(--ln)" }}>
          <img src={currentSlide.image} alt={`Slide ${slideIndex + 1}`} className="aspect-video w-full object-contain" style={{ background: "#111" }} />
        </div>
